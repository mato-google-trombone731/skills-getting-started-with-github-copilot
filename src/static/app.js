      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants && details.participants.length > 0) {
          participantsHtml = '<div class="participants"><strong>Participants:</strong><ul>' +
            details.participants.map(p => `<li><span class="participant-email">${p}</span><button class="remove-participant" data-activity="${name}" data-email="${p}" aria-label="Remove participant">✖</button></li>`).join('') +
            '</ul></div>';
        } else {
